...
| Field | Value |
|---|---|
| **Product name** | HORIZON GRID |
| **Version** | 0.3.14 |
| **Build** | Post-ship-audit follow-up fix pass, compiled 2026-08-14 14:09:17 |
| **Release date** | 2026-08-14 |
| **Git commit** | `f6fdac7380fb00066f79a4d36dc767738c7f292a` (current `main` HEAD; this working directory is a git repository) |
| **Installer** | `HORIZON-GRID-Setup-0.3.14.exe` |
...
- Frontend: Next.js 14.2.35 / React 18.3.1

## Supported OS

Windows 10/11, 64-bit only (`ArchitecturesAllowed=x64compatible` in the installer; no 32-bit build produced); also Linux (Debian 12, Ubuntu 22.04/24.04) via the `.deb` package.

## Database / Migration Version

Alembic head revision: `b3f0587f2493`

## Major Features

- Multi-provider IOC investigation (18 built-in threat-intelligence providers) with parallel querying, correlation, and AI-generated final assessment.
...
Ollama (local), Anthropic (Claude), Amazon Bedrock (Claude), Google Gemini, Groq, OpenAI (ChatGPT), Kimi (Moonshot AI), DeepSeek, xAI (Grok), Mistral AI, OpenRouter.

## IOC Providers (18)

VirusTotal, AbuseIPDB, AlienVault OTX, URLhaus, ThreatFox, MalwareBazaar, crt.sh, NIST NVD, CISA KEV, MITRE ATT&CK, WHOIS/RDAP, urlscan.io, Google Safe Browsing, Hybrid Analysis (Falcon Sandbox), Spamhaus, PhishTank, Censys, and the Internet Intelligence Collector.
...
- `documentation/HORIZON_GRID_USER_MANUAL.pdf` / `.docx` (22 sections, 56 figures)
- `documentation/HORIZON_GRID_SOURCE_CODE_GUIDE.pdf` / `.docx` (8 sections, 6 figures)
- `documentation/HORIZON_GRID_BACKEND_DOCUMENTATION.pdf` / `.docx` (8 sections, 9 figures)
