...
- **Live model discovery** — `POST /api/v1/ai/groq/models` calls Groq's own `/v1/models` endpoint at request time rather than hard-coding a model list that would go stale. Confirmed returning the account's real current model list.
