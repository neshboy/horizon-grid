...
def test_accepts_a_small_cidr_at_exactly_the_cap():
    """Real, globally-routable /28 (Google's public DNS range) -- exactly
    16 addresses, right at the cap. A private range would now (correctly)
    be rejected by the routability check added below; this test's own
    intent is the SIZE boundary, so it uses a public range to isolate that."""
