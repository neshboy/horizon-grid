...
- [👋 Welcome](#-welcome)
- [🔐 Setting Up the Administrator Account](#-setting-up-the-administrator-account)
- [🤖 Choosing an AI Model](#-choosing-an-ai-model)
- [🔌 Threat Intelligence Providers](#-threat-intelligence-providers)
- [Network Ports](#network-ports)
- [Reviewing the Summary](#reviewing-the-summary)
...
[FIGURE: 08-wizard-aiconfig.png | The AI Configuration page, where the administrator chooses which AI model will generate the AI-written summaries and final assessment for every investigation. (This screenshot predates several of the backends described below; the page now offers eleven choices in total -- Ollama plus ten cloud providers.)]
...
- **A cloud AI provider** instead, if you'd rather use a more capable hosted AI model from a third-party service. The wizard currently supports ten such providers: Anthropic, Amazon Bedrock, Google Gemini, Groq (a fast-inference API service), OpenAI, Kimi (Moonshot AI), DeepSeek, Grok (xAI -- a different product from Groq above, despite the similar name), Mistral AI, and OpenRouter (a meta-router giving access to hundreds of underlying models from many providers through one API). Each requires an account and an API key with that provider.
