...
The export controls for this live in the sidebar of every investigation page. Opening them shows four buttons: **Export JSON**, **Export Markdown**, **Export PDF**, and **Export CSV** — all four are real, working exports today.

[FIGURE: 21-investigation-export-menu.png | The export sidebar on an investigation page, showing all four export buttons.]

## What works today: all four formats

Clicking any of the four buttons downloads a real file to your browser's Downloads folder, built from the same investigation you're looking at on screen:
...
- **Export PDF** renders a branded, formatted report (`app/api/routes/lookup.py`'s `_render_pdf()`, via ReportLab) with a header/footer, suitable for attaching to an incident ticket or handing to someone who wants a document rather than raw text.
- **Export CSV** renders a spreadsheet-friendly row-based version, useful for bulk analysis or importing into another tool.
