...
- [🐳 1. What's actually running](#-1-whats-actually-running)
- [🩺 2. Day-to-day health checks](#-2-day-to-day-health-checks)
- [🗄️ 3. How provider-result caching works, operationally](#️-3-how-provider-result-caching-works-operationally)
- [⏱️ 4. What the background job actually does](#️-4-what-the-background-job-actually-does)
- [💾 5. Backup and restore](#-5-backup-and-restore)
- [📊 6. Monitoring Provider Health and the Executive Dashboard](#-6-monitoring-provider-health-and-the-executive-dashboard)
- [🔧 7. Database connection-pool sizing](#-7-database-connection-pool-sizing)
- [✅ Summary](#-summary)
...
1. **`GET /health`** (e.g. `http://localhost:8000/health`, no auth, outside `/api/v1`) -- the shallowest possible check. It returns `{"status":"ok","service":"HORIZON GRID"}` the instant Uvicorn accepts connections, and it does **not** check Postgres, Redis, Neo4j, or OpenSearch -- only that the backend process itself is up. This is what a Kubernetes readiness/liveness probe would hit in that topology, and what CI's own bare-runner smoke test uses. **Updated in the same mission-critical-reliability review as item 2 below:** the Setup Wizard's own post-install wait no longer polls this plain endpoint -- it was repointed at `GET /health/detailed` instead (up to three minutes), since the very next step writes to Postgres and needs to know it's genuinely reachable, not just that the process has started. A failure of the plain `/health` endpoint means the backend process itself is down or unreachable; it says nothing about whether the datastores behind it are healthy.
...
## 🗄️ 3. How provider-result caching works, operationally

Every IOC provider's result is cached in Redis (logical DB `/0`, shared with the lookup rate limiter) for **one hour by default** (`provider_cache_ttl_seconds`, `config.py:183`). The two operational facts worth internalizing:
...
## ⏱️ 4. What the background job actually does
...
**Updated by a later mission-critical-reliability review** (see the Mission-Critical Operations Manual for the full account): a real, scripted restore now exists on both platforms -- the Start Menu's **Restore Database** shortcut (`Restore-Database.ps1`) on Windows, and `sudo horizon-grid restore` on Linux -- both requiring a typed `RESTORE` confirmation unless `-Force`/`--force` is passed. It stops only the services that write to the database (`backend`, `celery_worker`, `celery_beat`), leaves Postgres running, drops and recreates the target database, replays the dump, then restarts what it stopped. The same review also added a nightly automatic backup on both platforms, on top of the existing automatic pre-upgrade backup already described above. The paragraph above describing restore as an unscripted, manual-only operation reflects the platform's state *before* that review and is kept for historical context, not current behavior.
