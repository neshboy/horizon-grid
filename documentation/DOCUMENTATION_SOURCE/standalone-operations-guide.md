...
|---|---|---|---|
| `app-backend-1` | built from `backend/` | FastAPI API, port 8000 -- runs `alembic upgrade head` then Uvicorn on every start | none -- a crash stays down until recreated |
| `app-frontend-1` | built from `frontend/` | Next.js UI, port 3000 | none |
| `app-postgres-1` | `postgres:16-alpine` | primary datastore -- investigations, cases, users, runtime provider config | none |
| `app-redis-1` | `redis:7-alpine` | provider-result cache, lookup rate limiter, Celery broker + result backend (three logical DB indexes on one process) | none |
| `app-neo4j-1` | `neo4j:5-community` | correlation graph (malware/threat-actor/campaign/CVE/MITRE-technique relationships) | none |
| `app-opensearch-1` | `opensearchproject/opensearch:2.17.0` | search/indexing backend | none |
