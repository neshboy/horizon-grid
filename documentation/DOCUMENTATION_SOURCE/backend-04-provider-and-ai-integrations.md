...
- **Normalized data**: `verdict` ("clean" if NXDOMAIN/no answer, "malicious" if any A-record hit), `listed` (bool), `list` (`"ZEN"` or `"DBL"`), `query`, and on a hit, `return_codes` (the returned `127.0.0.x` addresses) plus `listing_reason` (decoded from the published Spamhaus ZEN/DBL return-code tables, `:24-50`) (`:81-117`).
- **"Errors"**: `socket.gaierror` (NXDOMAIN) is caught and treated as "not listed" — a normal clean result, not an error (`:73-79`). Because there is no HTTP call, the 429/403/509 rate-limit mapping described in §1 does not apply to this connector.
