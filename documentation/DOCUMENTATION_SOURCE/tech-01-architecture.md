...
> "Unified threat intelligence workbench: single-search IOC lookup across dozens of providers, correlated and summarized by a local Ollama model (or AWS Bedrock/Gemini/Anthropic/Groq/OpenAI/Kimi/DeepSeek/xAI/Mistral/OpenRouter, configurable via AI_BACKEND)."
...
- [Backend](#-backend)
- [Frontend](#-frontend)
- [Executive Dashboard and Deterministic Scoring Engine](#-executive-dashboard-and-deterministic-scoring-engine)
- [Celery: Scope and a Key Boundary](#-celery-scope-and-a-key-boundary)
...
The backend is also where the 18 third-party data-source connectors ("providers" — 17 threat-intelligence sources plus the Internet Intelligence Collector OSINT crawler), the AI summarization/correlation logic, and the authentication/RBAC (role-based access control) system live — each of those is detailed in its own section of this document.
