...
The active backend is a single configuration value, `ai_backend` (`app/core/config.py` line 150), defaulting to `"ollama"`. Eleven backends are implemented:
...
The `AISummaryRecord.provider_id` column is the actual database-level marker distinguishing the two: a model-level comment (`models/lookup.py` line 111) explicitly documents that a `NULL` `provider_id` row is "the final consolidated assessment, not a per-provider one."
...
**`_ground_final_assessment()`** (service.py lines 273-326), run on every `generate_final_assessment()` result:
