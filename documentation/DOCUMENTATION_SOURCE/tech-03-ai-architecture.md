...
- [1. Supported AI Backends and Selection](#-1-supported-ai-backends-and-selection)
- [2. Runtime AI Backend Switching (No Restart)](#-2-runtime-ai-backend-switching-no-restart)
- [3. Two Distinct AI Call Types](#-3-two-distinct-ai-call-types)
- [4. No-Evidence Short-Circuit Guard](#-4-no-evidence-short-circuit-guard)
- [5. Grounding / Cross-Check Mechanism](#-5-grounding--cross-check-mechanism)
- [6. Verdict and Confidence Data Model](#-6-verdict-and-confidence-data-model)
- [7. AI Data Flow](#-7-ai-data-flow)
- [8. Beyond the Core Pipeline](#-8-beyond-the-core-pipeline)

---

## 🤖 1. Supported AI Backends and Selection

The active backend is a single configuration value, `ai_backend` (`app/core/config.py` line 158), defaulting to `"ollama"`. Eleven backends are implemented:
...
The `AISummaryRecord.provider_id` column is the actual database-level marker distinguishing the two: a model-level comment (`models/lookup.py` line 125) explicitly documents that a `NULL` `provider_id` row is "the final consolidated assessment, not a per-provider one."
...
Inside `generate_final_assessment()` (service.py lines 609-657), there is a hard guard: if `provider_summaries` is empty **and** `correlation.edges` is empty, the function returns a deterministic `FinalAssessment` — `final_verdict=Verdict.UNKNOWN`, with risk fields (`overall_risk_score`, `confidence_score`, `malicious_probability`, `severity`) sourced from the deterministic scoring engine's own result for this same evidence-free case rather than the AI backend, and an `ai_outcome` of `"skipped_no_evidence"` — **without invoking the AI backend at all**. In the ordinary all-zero-evidence case the scoring engine itself produces all-zero values here, but the guard deliberately reads those values from the scoring engine rather than hardcoding them, so it also stays correct in the rarer edge case where, e.g., a Security Assessment finding has already raised the score with no accompanying provider summary or correlation edge.
...
**`_ground_final_assessment()`** (service.py lines 376-429), run on every `generate_final_assessment()` result:
...
- `_strip_invalid_evidence_ids()` (lines 71-87) recursively removes any `evidence_ids` citation that doesn't match a real `EvidenceItem.id` belonging to that specific lookup.
- `_backfill_evidence_ids_from_prose()` (lines 90-128) does the reverse repair: it recovers citations the model correctly referenced in free-text prose but failed to place into the structured `evidence_ids` field — but it only ever adds an ID that has already been confirmed real; it never invents one.
...
The `Verdict` enum (`app/models/lookup.py` lines 22-34) has exactly **12 values**: `highly_malicious`, `malicious`, `suspicious`, `unknown`, `likely_benign`, `benign`, `scanner`, `tor_exit_node`, `vpn`, `cdn`, `cloud_infrastructure`, `dormant_infrastructure`.

> [!NOTE]
> Several of these are not "maliciousness" gradations at all but infrastructure classifications (`tor_exit_node`, `vpn`, `cdn`, `cloud_infrastructure`, `dormant_infrastructure`) — the model is expected to distinguish "this is malicious" from "this is a category of infrastructure that merits a different interpretation."

`RiskAssessment` (`app/ai/schemas.py` lines 111-152) carries `overall_risk_score`, `confidence_score`, and `malicious_probability`, all explicitly on a **0-100 scale** (not 0-1). A `field_validator` automatically rescales any value it receives between 0 and 1 by multiplying by 100 — a defensive fix added because `llama3.2:3b` was observed defaulting to a 0-1 convention despite the 0-100 instruction.

`FinalAssessment` additionally carries a `model_validator` named `_verdict_must_agree_with_risk` (schemas.py lines 227-248) that rejects egregiously self-contradictory combinations — for example, a `final_verdict="malicious"` paired with `malicious_probability < 30` is rejected rather than silently persisted. This is a second, independent layer of internal-consistency checking, separate from the evidence-grounding checks in §5.
