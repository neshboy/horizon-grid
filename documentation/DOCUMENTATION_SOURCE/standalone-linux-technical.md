...
HORIZON GRID on Linux is distributed as a `.deb` package (`horizon-grid_0.2.0_amd64.deb`) that installs a systemd unit and a `horizon-grid` CLI, including a terminal setup wizard. This is the direct Linux analog of the Windows installer + WinForms wizard + Docker Compose combination that already exists for that platform.
...
5. After building, verify the resulting `.deb`'s size is in the same ~6 MB range as `horizon-grid_0.2.0_amd64.deb` (5.8 MiB); a much larger artifact likely indicates a vendored-dependency or virtualenv regression.
...
8. Version numbers (package version, `MyAppVersion`, backend, frontend) must be kept in lockstep across platforms — the current release is `0.2.0` on both Windows and Linux, with no version drift.
