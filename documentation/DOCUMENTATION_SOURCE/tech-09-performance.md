...
| AI-backend client timeout, Ollama (`_TIMEOUT_SECONDS`) — applies only when `ai_backend=ollama` (the default); no equivalent hardcoded value is documented for the Anthropic/Bedrock/Gemini clients | 120 seconds, hardcoded | Configured limit | architecture-facts.md §9 |
