...
| AI-backend client timeout, Ollama (`ollama_timeout_seconds`, default `300`; `_TIMEOUT_SECONDS` in `ollama_client.py` is only a same-valued fallback used if a caller constructs the client without a Settings object) — applies only when `ai_backend=ollama` (the default) | 300 seconds, configurable | Configured limit | architecture-facts.md §9 |
| AI-backend client timeout, the other ten backends — nine (Anthropic, Gemini, Groq, OpenAI, Kimi, DeepSeek, xAI, Mistral, OpenRouter) hardcode their own 60-second client-side timeout; Bedrock's client sets its own 120-second `read_timeout` via `BotoConfig`, overriding the AWS SDK's 60-second default | 60 seconds, hardcoded, for the nine non-Bedrock cloud backends; 120 seconds, hardcoded, for Bedrock | Configured limit | direct inspection of `backend/app/ai/*_client.py` |
...
- **The 300-second timeout is Ollama-specific.** It applies when the platform is configured to use the default local `ollama` AI backend (model `llama3.2:3b`). Nine of the ten other backend clients (`anthropic`, `gemini`, `groq`, `openai`, `kimi`, `deepseek`, `xai`, `mistral`, `openrouter`) each hardcode their own, shorter 60-second client timeout; `bedrock`'s client sets its own 120-second `read_timeout` via `BotoConfig`, longer than the AWS SDK's 60-second default.
