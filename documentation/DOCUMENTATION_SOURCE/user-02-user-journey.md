...
An admin installs the platform via the Windows or Linux installer, then runs a setup wizard (a native desktop app on Windows, a terminal wizard on Linux) collecting an administrator account, an AI backend choice, and API keys for whichever threat-intelligence providers are available. After that, day-to-day use is just opening a browser tab.
...
Investigations export cleanly as JSON, Markdown, PDF, or CSV.
