...
Compose is the only deployment path fully wired end-to-end and exercised by this project's own test suite. `docker-compose.yml` defines eight services: four datastores (`postgres`, `redis`, `neo4j`, `opensearch`, all published to `127.0.0.1` only -- an explicit fix after confirming each was reachable unauthenticated from other LAN devices under the old `0.0.0.0` shorthand), `backend`, two Celery roles (`celery_worker`, `celery_beat`), and `frontend`. `backend` and `celery_worker` wait on Postgres/Redis health checks (`pg_isready`/`redis-cli ping`, 5s interval, 10 retries); `celery_beat` waits only on Redis. All eight services carry `restart: unless-stopped`, so a crashed/OOM-killed container comes back on its own rather than staying down until manually recreated (an explicit `docker stop`/`docker compose stop` is still respected, since `unless-stopped`, unlike `always`, does not fight an intentional stop).

> [!NOTE]
> All eight services carry `restart: unless-stopped`. A crashed or OOM-killed container is automatically recreated; an operator-initiated `docker compose stop` is still respected and won't be auto-restarted.
...
A plain-YAML, kustomize-compatible, hand-translated mirror of the Compose services also exists, with no Helm templating and no CI/CD wiring anywhere in the repo. `backend`/`frontend`/`celery-worker` run 2 replicas; `celery-beat` is pinned to 1 (avoids duplicate scheduled dispatches). `backend` probes `GET /health/detailed` for readiness and liveness; resource requests/limits (`backend`: `250m`/`512Mi` request, `1000m`/`4Gi` limit; `celery-worker`: `250m`/`512Mi` request, `1000m`/`1Gi` limit; `frontend`: `100m`/`256Mi` request, `500m`/`512Mi` limit) are static manifest values, not load-test-derived -- `backend`'s limit is higher than `celery-worker`'s to match `docker-compose.yml`'s own 4g `backend` `mem_limit` for the Metasploit Framework install and its `msfrpcd` daemon. TLS is commented out in `ingress.yaml` and not implemented. `frontend/Dockerfile`'s baked-in `CMD` is dev-mode only (`npm run dev`), but `frontend-deployment.yaml` overrides the container `command:` to build and serve the standalone output directly (`npm run build && ... && node .next/standalone/server.js`, the same fix `docker-compose.prod.yml` uses) rather than relying on that `CMD` or a separate production Dockerfile.

**Migrations in this topology.** Unlike Compose's single-container `alembic upgrade head && uvicorn ...` command, migrations here run via a separate, single-run `backend-migrate` Kubernetes `Job` (`k8s/base/backend-deployment.yaml:50-74`, `command: ["alembic", "upgrade", "head"]`) that is applied and awaited before the `backend` Deployment itself -- fixing a real race that used to exist here: with `replicas: 2`, both of the Deployment's own pods previously ran `alembic upgrade head` inline on every start, with nothing serializing the two invocations against each other. The Deployment's pods (line 109) now only run `msfrpcd -f -P "${MSF_RPC_PASSWORD}" -S -a 127.0.0.1 -p 55553 -U msf & uvicorn app.main:app --host 0.0.0.0 --port 8000` -- no `alembic` invocation at all. `celery-worker`'s Deployment still does not run `alembic upgrade head` (same division of responsibility as Compose), so it depends on the `backend-migrate` Job having already brought the schema current before either `backend` or `celery-worker` starts.
...
|---|---|---|
| App | `debug`, `environment` | `debug` defaults `True` but has no effect anywhere in the code (dead setting); CORS is governed independently by a hardcoded private-network origin regex (any `localhost`/loopback/RFC 1918 host, any port -- see `main.py`'s `PRIVATE_NETWORK_ORIGIN_REGEX`), and `/docs`/`/redoc`/the OpenAPI schema are gated by `environment == "production"`, not by `debug` |
...
| Rate limiting | `lookup_rate_limit_max_calls`/`_window_seconds`, `login_rate_limit_max_attempts`/`_window_seconds` | `10`/`60` for both the lookup limiter and the login/register limiter |
...
1. **`docker compose ps`** -- which containers are actually running vs. exited (all eight now carry `restart: unless-stopped`, so a container repeatedly cycling rather than settling `Up` is itself a symptom).
...
3. **`GET /health`** (no auth, outside `/api/v1`) -- `{"status": "ok", "service": "HORIZON GRID"}` the moment Uvicorn accepts connections; it does **not** check Postgres/Redis/Neo4j/OpenSearch, only that the process itself is up. `GET /health/detailed` (genuinely checks Postgres/Redis) is what the Setup Wizard and the Compose/Kubernetes readiness/liveness probes actually poll.
...
AI backend failures raise `RuntimeError` from the relevant client, caught by `app/ai/service.py` and converted to a degraded static result (verdict `unknown`, scores `0`) rather than failing the lookup. Only Bedrock retries (`botocore`-level, `BotoConfig(retries={"max_attempts": 3, "mode": "adaptive"})`); every other backend makes exactly one attempt. Common real messages: `"Could not reach Ollama at http://host.docker.internal:11434 -- is it running?"` (Ollama not running on the host -- it's deliberately never containerized, since a large model's mmap load was observed dramatically slower through Docker Desktop's WSL2 volume layer); `"Ollama did not respond within 300s (model=llama3.2:3b) -- likely too slow for available hardware"` (`OLLAMA_TIMEOUT_SECONDS`, defaulting to `300`, is a real `.env`-overridable setting now -- raise it for a genuinely slow/larger model, or switch to a smaller one); and, for any backend, `"AI backend '{backend}' is not configured (missing API key/URL/model) -- ..."` if `is_configured` is still false after resolution.

**`429` on `POST /api/v1/lookup/stream`** comes from a Redis-backed fixed-window `RateLimiter` keyed `lookup_create:{user.id}`, per user across all workers, bounded by `lookup_rate_limit_max_calls`/`_window_seconds` (`10`/`60`). Exists because one lookup fans out to every provider plus the crawler plus multiple AI calls. `POST /api/v1/auth/login` and `POST /api/v1/auth/register` each carry their own `RateLimiter` too, keyed by the attempted email address and bounded by `login_rate_limit_max_attempts`/`_window_seconds` (`10`/`60`) -- no other endpoint has any rate limiting.
...
- **DB connection pool**: `backend/app/core/db.py` creates one process-wide `AsyncEngine` (`create_async_engine(..., pool_pre_ping=True, echo=False, pool_size=db_pool_size, max_overflow=db_pool_max_overflow)`), shared by `get_db()` and `new_session()`. `db_pool_size`/`db_pool_max_overflow` default to `5`/`5` in `config.py`, but `docker-compose.yml`'s `backend` service (only -- `celery_worker`/`celery_beat` keep the smaller defaults) overrides them to `30`/`20` via `DB_POOL_SIZE`/`DB_POOL_MAX_OVERFLOW`, matched by the same override on the `k8s/base` `backend` Deployment.
...
- **Celery concurrency**: `docker-compose.yml`'s `celery_worker` command sets `--concurrency=${CELERY_WORKER_CONCURRENCY:-4}` (mirrored in `k8s/base/celery-worker-deployment.yaml`) -- a fixed default of `4`, not Celery's own per-CPU-core default, chosen to avoid the container's `mem_limit` being exhausted by per-process overhead on high-core hosts. The interactive SSE lookup path never goes through Celery at all, so this has no bearing on lookup latency.
- **Kubernetes-only figures**: `backend` requests `250m`/`512Mi` (limit `1000m`/`4Gi`, sized for the Metasploit Framework install and its `msfrpcd` daemon); `celery-worker` requests `250m`/`512Mi` (limit `1000m`/`1Gi`); `frontend` requests `100m`/`256Mi` (limit `500m`/`512Mi`); `backend`/`frontend`/`celery-worker` run 2 replicas, `celery-beat` is a pinned singleton. Static manifest values, not load-derived.
