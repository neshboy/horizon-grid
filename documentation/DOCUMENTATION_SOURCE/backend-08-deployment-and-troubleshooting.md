...
On "Start Installation" the wizard runs, in order: (1) writes `.env` via `Write-PlatformEnvFile`, using collected values plus CSPRNG-generated secrets (`ConvertTo-SafeEnvValue` strips CR/LF and rejects any value containing `#`, since `#` opens a `.env` comment and would silently truncate the rest); (2) runs `docker compose -f docker-compose.yml -f docker-compose.prod.yml up -d --build` -- **this is the moment Docker actually starts**, nothing before it brings up a single container; (3) polls `GET /health` for up to 3 minutes; (4) registers the admin account via `POST /api/v1/auth/register` (first-ever registration becomes `admin`, every later one `analyst`) and logs in. Re-running the wizard via the **Configuration** shortcut on an existing install pre-populates fields from the existing `.env` and takes a `pg_dump` backup first.
