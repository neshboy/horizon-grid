...
Authentication is JWT-based. `backend/app/auth/security.py` issues tokens with `python-jose`, signing algorithm `HS256` (`jwt_algorithm` in `app/core/config.py:30`), and hashes passwords with `passlib`'s `bcrypt` scheme (`security.py:10`) — plaintext passwords are never persisted, only the bcrypt hash on `users.hashed_password`.
...
|---|---|---|---|
| Access token | `"access"` | 30 minutes | `access_token_expire_minutes` (`config.py:31`) |
| Refresh token | `"refresh"` | 7 days | `refresh_token_expire_days` (`config.py:32`) |

`POST /api/v1/auth/refresh` (`app/api/routes/auth.py:205-223`) only accepts a token whose decoded `type` claim is `"refresh"`; `get_current_user` (`app/auth/rbac.py:29-71`), used by every protected route, only accepts `type == "access"` — a refresh token cannot be used directly as a bearer credential, and vice versa. `get_current_user` re-checks `user.is_active` against the database on every request (`rbac.py:48`), so an account disabled after a token was issued is rejected on its very next request, without waiting for the token to expire.
...
| `LoginRequest.password` | `max_length=72` only (no minimum) | `app/schemas/auth.py:30` |
...
- **Self-service session revocation.** `POST /auth/logout` (`app/api/routes/auth.py:231-246`) increments the caller's own `token_version`, immediately invalidating every previously issued access/refresh token for that account ("log out everywhere") without requiring a password change.
- **Both `/auth/login` and `/auth/register` have real per-account rate limiters**, added in a later mission-critical-reliability review (v0.2.3): Redis fixed-window limiters (`app/core/cache.py::RateLimiter`), keyed `login:{email}` and `register:{email}` respectively rather than by source IP, since registration/login lookups are already keyed by email throughout the codebase. Each rejects further attempts against the same address with `429` once `login_rate_limit_max_attempts` (10 by default) is exceeded within `login_rate_limit_window_seconds` (60 by default), both configurable — `/auth/register` reuses these same two settings rather than having its own dedicated ones.

**Recommended (not yet implemented):**
- **No revocation of a single session.** `token_version` (`app/models/user.py`) is a single counter per user, not a per-session identifier: both the admin-driven `POST /api/v1/admin/users/{id}/reset-password` (§2's user-management subsection) and the self-service `POST /auth/logout` above invalidate *every* outstanding token for that account at once — there is no way to revoke one specific device/session while leaving others logged in, and there is still no revocation for reasons other than a password reset or a self-service logout short of rotating `jwt_secret_key`, which invalidates *every* session platform-wide.
...
The platform has exactly three roles, `Role.ADMIN`, `Role.ANALYST`, `Role.VIEWER` (`app/models/user.py:13-16`), stored on `users.role`. Authorization is a flat permission-string matrix, `ROLE_PERMISSIONS` (`app/models/user.py:48-76`), checked per-route by the `require_permission(permission: str)` dependency factory (`app/auth/rbac.py:74-86`): a route either declares a required permission string as a FastAPI dependency or it enforces none — there is no implicit default-deny/default-allow fallback beyond "no `require_permission` call means no permission check."

| Permission string | admin | analyst | viewer | Enforced by |
|---|---|---|---|---|
| `lookup:create` | ✓ | ✓ | | `POST /lookup/stream`, `POST /lookup/{id}/reanalyze` |
| `lookup:read` | ✓ | ✓ | ✓ | `GET /lookup/{id}`, `GET /lookup`, `GET /lookup/{id}/assessments`, `GET /lookup/{id}/pivots`, `GET /runtime/ai-active`, `GET /runtime/ai-providers` |
| `lookup:export` | ✓ | ✓ | | `POST /lookup/{id}/export` |
| `evidence:read` | ✓ | ✓ | ✓ | `GET /lookup/{id}/analysis/evidence` |
| `analysis:generate` | ✓ | ✓ | | all eight AI explanation endpoints under `/lookup/{id}/analysis/*` (why, what-is-this, disagreement, false-positive, challenge, next-actions, gaps, score-explanation) |
...
| `provider:manage` | ✓ | | | `POST /providers/{id}/test`, `POST /ai/test`, `POST /ai/{backend}/models`, every `/runtime/ioc-providers*` route, and every `/runtime/ai-providers*` route except the read-only `GET`, which is `lookup:read` |
...
- **Self-role-change and self-deactivation are both rejected** (`400`, `SelfRoleChangeError`/`SelfDeactivationError`) — `if user.id == actor_user_id` in
  `update_user()`, and the equivalent guard in `set_user_active()` — purely to prevent an accidental mid-session self-lockout, since role changes and disables take
...
- **Last-admin protection is race-safe, not just check-then-act.** `_lock_all_admin_rows()`
  (`users.py:80-85`) takes a `SELECT ... FOR UPDATE` lock on **every** `ADMIN`-role row before
  `_remaining_active_admins()` (`users.py:88-89`) counts how many would stay active. A lock scoped to
...
- Race-safe last-administrator protection and self-role-change/self-deactivation prevention.
...
**Path A — `.env` (legacy, process-frozen).** `Settings` (`app/core/config.py:13-230`) is a `pydantic-settings` model loaded once from `.env` and cached for the process lifetime via `@lru_cache def get_settings()`. Every provider/AI credential field here (`virustotal_api_key`, `anthropic_api_key`, `jwt_secret_key`, etc.) is fixed at process start; changing `.env` requires a container restart to take effect for anything still reading `get_settings()` directly.

**Path B — DB-backed runtime configuration (encrypted).** The `provider_runtime_configs` table (`app/models/runtime_config.py:30-55`) lets an administrator add/change AI-backend and IOC-provider credentials through the Manage Providers UI while the platform is running, with no restart. Because this moves secret entry out of the git-ignored `.env` file into a database row, the credential dict is Fernet-encrypted before it is ever written: `app/core/crypto.py:52-57` (`encrypt_secret`), called from `runtime_config.py:337`/`:493`, storing ciphertext in `encrypted_credentials` (`Text`, nullable). Decryption (`crypto.py:60-84`) returns `""` on any `InvalidToken` rather than raising — a corrupted row or rotated key degrades to "not configured," not a 500.

### Key material and the HKDF fallback (a documented tradeoff, not a gap)

The Fernet key is resolved by `_fernet()` (`crypto.py:42-49`): if `settings.encryption_master_key` is explicitly set, that value is the key; otherwise a key is deterministically derived via HKDF-SHA256 from `settings.jwt_secret_key` (`crypto.py:36-39`), domain-separated by a fixed `info` string so it cannot collide with any other derivation of that same secret. The module's own docstring states the intent plainly (`crypto.py:1-20`): this fallback means every existing install already has a usable encryption key with zero migration effort, at the cost of key *independence* — compromising `jwt_secret_key` also exposes the derived Fernet key. That is defense-in-depth against a raw database leak, not HSM-grade key separation, and it is a documented tradeoff rather than an oversight: an operator wanting true separation sets `encryption_master_key` explicitly (`config.py:36`).

### Masked display — the API never returns a raw runtime-configured secret

Every read path for a runtime-configured provider (`GET /runtime/ai-providers`, `GET /runtime/ioc-providers`) goes through `_row_to_public_dict()` (`app/core/runtime_config.py:194-233`), which decrypts internally only to immediately re-mask: `masked_credentials = {k: mask_secret(v) for k, v in creds.items()}`. `mask_secret()` (`crypto.py:87-94`) replaces every character except the last four with `*` and is documented as intentionally non-reversible ("never round-trippable back to the real value, unlike returning a truncated real prefix"). No route in `app/api/routes/runtime.py` returns a decrypted credential to a client.

### Windows installer: file-level protection for `.env`

For Path A, the Windows Setup Wizard does two things a plain `.env.example`-copy would not. It generates `jwt_secret_key` (and the Postgres/Neo4j passwords) with .NET's cryptographically secure `RandomNumberGenerator`, not PowerShell's `Get-Random` (`windows/scripts/Common.ps1`'s `New-RandomSecret`, lines 118-126, called from `windows/scripts/Write-EnvFile.ps1`'s `New-DefaultPlatformSettings`, lines 156-215). It then restricts the written file's ACL immediately: `icacls.exe $Path /inheritance:r /grant:r "*S-1-5-32-544:F" "*S-1-5-18:F"` (`Write-EnvFile.ps1:153`) — breaking inheritance and granting Full Control only to Administrators (`S-1-5-32-544`) and SYSTEM (`S-1-5-18`); the same pattern is reused for the data directory (`windows/scripts/Common.ps1:106`) and for the `.env` copy Compose actually reads from (`Common.ps1:304`). A separate fix, `ConvertTo-SafeEnvValue` (`Write-EnvFile.ps1:15-49`), strips CR/LF from every value and rejects any value containing `#` outright, since `#` starts a comment in `.env` syntax and would otherwise silently truncate a credential with no error downstream.
...
- `seed_from_env_if_empty()` (`app/core/runtime_config.py:587-637`) migrates existing `.env` credentials into the encrypted store exactly once, idempotently, on first startup of this feature — an upgrade never silently drops a previously configured key.

**Recommended (not yet implemented):**
- **`jwt_secret_key` defaults to the literal string `"change-me-in-production"`** if never set (`config.py:29`). Because the Fernet key for Path B falls back to a derivation of this same value, an unedited default does not just weaken JWT signing — by the tradeoff above, it also weakens runtime-credential encryption for any deployment that never sets `encryption_master_key`. (The Windows installer avoids this by always generating a real value; a bare `docker compose up` against `.env.example` without wizard involvement does not.)
...
| `user.create` | `create_user` (`users.py:177-200`) | Yes |
| `user.update` | `update_user` (`users.py:203-242`) — only written if `full_name`/`role` actually changed | Yes |
| `user.enable` / `.disable` | `set_user_active` (`users.py:245-281`) | Yes |
| `user.password_reset` | `reset_password` (`users.py:284-308`) | Yes (the administrator performing the reset, never the affected user) |
| `auth.login` | `record_login_success`, called from `POST /auth/login` (`users.py:311-317`) | Yes — the logging-in user is recorded as their own actor |
| `auth.login_failed` | `record_login_failure`, called from `POST /auth/login` on bad credentials (`users.py:326-327`) | No (`actor_user_id=None`) — the email that was attempted is in `detail`, but there is no authenticated identity to attribute it to; the function's signature (`email: str` only) makes it structurally impossible to pass a password into `detail` |
...
|---|---|---|
| Authentication | JWT (HS256) + bcrypt, DB-checked `is_active` on every request; `token_version`-based immediate invalidation on admin-initiated password reset and self-service `/auth/logout`; per-account rate limiters on both `/auth/login` and `/auth/register` (v0.2.3) | No revocation of a single session (only all-or-nothing); no account lockout on login |
