...
|---|---|---|---|
| Unit tests | `backend/app/tests/unit/` | 65 files | ~11,480 lines |
| Integration tests | `backend/app/tests/integration/` | 34 files | ~11,171 lines |
...
The frontend (Next.js/React/TypeScript) declares a `"test": "vitest run"` script in `package.json` and lists Vitest as a devDependency, backed by a real `vitest.config.ts`.

**A repo-wide search for `*.test.*` / `*.spec.*` files under `frontend/` (excluding `node_modules` and `.next`) found 5 test files** — `frontend/lib/api.test.ts`, `frontend/lib/authedFetch.test.ts`, `frontend/lib/dashboardSummary.test.ts`, `frontend/lib/runEffectOnce.test.ts`, and `frontend/app/pentest/page.test.ts` — totaling **30 passing tests** (`npx vitest run`: 5 test files passed, 30 tests passed). These are plain-function/regression-style unit tests (login/authedFetch error and session-recovery handling, a dashboard loading-hint helper, a Strict-Mode-safe effect-once guard, and a static source assertion that the Pentest kill-switch UI is wired up) — `vitest.config.ts`'s own comment notes the suite runs in a plain Node environment with no jsdom/React Testing Library wired in yet, so no test actually renders a React component.

### 1.3 Summary

| Area | Framework(s) configured | Test files that actually exist | Coverage % |
|---|---|---|---|
| Backend unit | pytest, pytest-asyncio, pytest-cov | 65 | Not measured |
| Backend integration | pytest, respx, real Redis | 34 | Not measured |
| Frontend | Vitest | 5 (30 tests) | Not measured |
