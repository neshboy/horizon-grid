...
| Permission | Status |
|---|---|
| `provider:manage` | Enforced — gates `POST /api/v1/providers/{provider_id}/test` and the new `/api/v1/runtime/*` provider- and AI-configuration endpoints |
| `user:manage` | Defined in the RBAC matrix; no route currently calls `require_permission("user:manage")` |
