...
if docker exec "$CONTAINER_ID" pg_dump -U "$PG_USER" "$PG_DB" >"$DUMP_PATH" 2>/dev/null && [ -s "$DUMP_PATH" ]; then
    chmod 600 "$DUMP_PATH"
    ...
    hg_log "Database backup failed or produced an empty file: $DUMP_PATH" "ERROR"
